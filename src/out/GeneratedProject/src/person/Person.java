...
public class Person {
    ...
    public void save(){
        openSqLite();
        if(this.id >= 1){
            String sql = String.format("UPDATE Person SET __name__ = '%s',name = '%s',age = '%s' WHERE id = '%s'",this.__name__,this.name,this.age,this.id);
            sqLiteConn.executeUpdate(sql);
        }else{
            String sql = String.format("INSERT INTO Person (__name__,name,age) VALUES ('%s','%s','%s')",this.__name__,this.name,this.age);
