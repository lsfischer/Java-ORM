...
public class Book {
    ...
    public void save(){
        ...
        openSqLite();
        if(this.id >= 1){
            String sql = String.format("UPDATE Book SET __name__ = '%s',title = '%s',pubDate = '%s',price = '%s',quantity = '%s' WHERE id = '%s'",this.__name__,this.title,this.pubDate,this.price,this.quantity,this.id);
            sqLiteConn.executeUpdate(sql);
        }else{
            String sql = String.format("INSERT INTO Book (__name__,title,pubDate,price,quantity) VALUES ('%s','%s','%s','%s','%s')",this.__name__,this.title,this.pubDate,this.price,this.quantity);
